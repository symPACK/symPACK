...
int main(int argc, char *argv[]) 
{
  ...
  for (int i = upcxx::myrank(); i < n; i += upcxx::ranks()) {
    ...
    my_node_t * node_ptr = cur_node;
    // cur_node.id = i+1;
    node_ptr->id = i+1;
    node_ptr->degree = xadj[i+1] - xadj[i];
    node_ptr->elim_step = -1;
...
//    memberof(cur_node, elim_step) = -1;
    ...
    node_ptr->adj_sz = adj_sz;
    node_ptr->adj = new std::set<int>();
    for (int j=0; j<adj_sz; j++) {
      node_ptr->adj->insert(local_adj[xadj[i] - 1 + j]);
    }
  }
  ...
  for (int step=1; step<=n; ++step) {
    ...
    int min_adj_sz =memberof(min_ptr,adj_sz); 
    if(upcxx::myrank()==min_ptr.where()){
      my_node_t * node_ptr = min_ptr;
      reach.insert(reach.end(),node_ptr->adj->begin(),node_ptr->adj->end());
   //   cout<<"Reach of "<<global_min_id<<": ";for(int i=0;i<reach.size();++i){cout<<reach[i]<<" ";}cout<<endl;

    }
    else{
      reach.resize(min_adj_sz);
    }
    
    upcxx::upcxx_bcast(&reach[0], &reach[0], min_adj_sz*sizeof(int), min_ptr.where());
...
#pragma omp parallel for
   ...
   //   cout<<"\tCur Reach of "<<node_ptr->id<<": ";for(auto i=node_ptr->adj->begin();i!=node_ptr->adj->end();++i){cout<<*i<<" ";}cout<<endl;

      node_ptr->adj->insert(reach.begin(),reach.end());
      node_ptr->adj->erase(global_min_id);


   //   cout<<"\tNew Reach of "<<node_ptr->id<<": ";for(auto i=node_ptr->adj->begin();i!=node_ptr->adj->end();++i){cout<<*i<<" ";}cout<<endl;


      int new_adj_sz = node_ptr->adj->size();      
