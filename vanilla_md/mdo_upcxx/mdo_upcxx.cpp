...
TIMER_START(mdo_time);
  ...
  for (int step=1; step<=n; ++step) {
    ...
    if((&nodes[global_min_id-1]).where()==upcxx::myrank()){
      node_t cur_node = nodes[global_min_id-1];
      int * local_adj = cur_node.adj;
      for(int i = 0; i< cur_node.adj_sz; ++i){
        assert(local_adj[i]>0);
      }
    }
