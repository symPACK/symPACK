/**
 * @file iohb.c
 * @author K.A. Remington, edited by Mark Hoemmen
 * @version 1.1
 * @date Time-stamp: <2008-07-16 11:23:30 mhoemmen>
 *
 * Harwell-Boeing sparse matrix file I/O routines.  
 * Mark Hoemmen (mfh) made some modest bug fixes (e.g. improving robustness of
 * the header parsing routines), and changed _SP_base to zero.
 */
...
/*************************************************/
    ...
    fmt = mfh_upcase (fmt);
    tmp = strchr(fmt,'(');
    len = (int) (strchr (fmt, 'I') - tmp - 1);
    if (len <= 0)
      {
	*perline = 1; /* Default if no perline is given in the format spec */
      }
    else
      {
    ...
    *width = atoi(tmp);
    ...
    free (fmt); /* it points to a new string, alloc'd by mfh_upcase */
    return *width;
}
...
/*************************************************/
    ...
    tmp = strchr(fmt,'(');
    {
      int len = (int) (strchr (fmt, *flag) - tmp - 1);
      if (len <= 0)
	*perline = 1; /* default if not specified */
      else
	{
	  tmp = substr (fmt,tmp - fmt + 1, len);
	  *perline = atoi(tmp);
