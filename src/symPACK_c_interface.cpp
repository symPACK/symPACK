...
extern "C" 
  int symPACK_C_InitInstanceDoubleComplex(MPI_Comm ccomm, bool is2D){
    ...
    symPACK_handle handle;
    symPACKOptions & optionsFact = handle.options;
    optionsFact.MPIcomm = ccomm;
    optionsFact.decomposition = DecompositionType::LDL;
    optionsFact.verbose = 0;
...
extern "C" 
  void symPACK_C_SetOptions(int * sp_handle, 
      MPI_Comm ccomm, 
      int * maxSize,
      int * verbose,
      int * NpOrdering,
      int * numThreads,
      const char * orderingStr,
      const char * decompTypeStr, 
      const char * mappingTypeStr,
      const char * load_balance_str,
      const char * order_refinement_str
      ){
    symPACK_handle & handle = symPACK_handles[*sp_handle];

    symPACKOptions & optionsFact = handle.options;
    optionsFact.MPIcomm = ccomm;
    if ( decompTypeStr == std::string("LDL") ){
      optionsFact.decomposition = DecompositionType::LDL;
    } else {
      optionsFact.decomposition = DecompositionType::LL;
    }

    optionsFact.relax.SetMaxSize(*maxSize);
    optionsFact.verbose = *verbose;
    optionsFact.order_refinement_str = order_refinement_str;
      optionsFact.load_balance_str     = load_balance_str;
      optionsFact.mappingTypeStr       = mappingTypeStr;
      optionsFact.orderingStr          = orderingStr;
      optionsFact.numThreads           = *numThreads;
      optionsFact.NpOrdering           = *NpOrdering;
      optionsFact.verbose              = *verbose;

      symPACK_handles[*sp_handle].factorMat->Init(optionsFact);
  }

extern "C" 
  void symPACK_SetOptions(
      int * sp_handle, 
      MPI_Fint * Fcomm,
     int * maxSize, 
      int * verbose,
