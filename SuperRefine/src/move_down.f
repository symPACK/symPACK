********************************************************************************
***  void move_down (int *heap, int heap_size, int fld_index, int *vtx_to_heap)
***  {
...
***  
***  		register int fld_next1, fld_next2, fld_now, fld_next,
***  			 val1, val2, val, vtx;
***  
***  		for (fld_now = fld_index; 
***  			(fld_now <= heap_size); ) {
***  			
***  			val = heap[( fld_now <<1)-1]; 
***  
***  			fld_next1 = (fld_now <<1);
***  			fld_next2 = fld_next1+1;
***  
***  			if (fld_next1 <= heap_size) 
***  				val1 = heap[( fld_next1 <<1)-1]; 
***  			else val1 = infty;
***  
***  			if (fld_next2 <= heap_size) 
***  				val2 = heap[( fld_next2 <<1)-1]; 
***  			else val2 = infty;
***  
***  			if ((val <=val1) && (val <= val2)) fld_now = heap_size+1;
***  			else  {
***  				if (val1 <=val2) fld_next =fld_next1;
***  			        else fld_next =fld_next2;
***  
***  				/*swap*/
***  				
***  				fld_next1 = (fld_now <<1) ;
***  				fld_next2 = (fld_next <<1);
***  
***  				vtx = heap[fld_next1]; 
***  
