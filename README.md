...
- `-gpu_mem <SIZE>` controls the amount of memory allocated on the GPU upon calling the `symPACK_cuda_setup()` method. One can optionally supply a "MiB" or "GiB" suffix (e.g. -gpu\_mem 2GiB). If no suffix is included, the allocation size will be interpreted in terms of bytes.  
