...
class SubtreeToSubcube: public LoadBalancer{
  ...
  protected:
    ...
    IntNumVec & cc_;
  public:
      SubtreeToSubcube(Int np, ETree & supETree,IntNumVec & Xsuper, IntNumVec & SupMembership,PtrVec & Xlindx, IdxVec & Lindx, IntNumVec & pCc):Xsuper_(Xsuper),SupMembership_(SupMembership),cc_(pCc),Xlindx_(Xlindx),Lindx_(Lindx),LoadBalancer(np,supETree.Size()){
          supETree_ = supETree;
...
#if 1
            {
              ...
              for(Ptr rowptr = fi; rowptr<=li;rowptr++){
                ...
                if(tgt_snode_id!=I){
                  ...
                  double update_cost = 2.0*m*n*k;
                  //SubTreeLoad[tgt_snode_id]+=update_cost;
...
//                SubTreeLoad[tgt_snode_id]+=update_cost;
          ...
          for(Int I=n_; I>= 1;I--){ 
            ...
            if(parent!=0){
              ...
              Int height = cc_[fc-1];
              parent_load = CHOLESKY_COST(height,width);
