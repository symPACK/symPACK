...
inline Int SuperNode<T,Allocator>::Shrink(){
  if(meta_->b_own_storage_){
    ...
    if(block_space >0 || nzval_space >0){
      ...
      size_t offset_block = meta_->nzval_cnt_*sizeof(T)+sizeof(SuperNodeDesc);//+meta_->blocks_cnt_*sizeof(NZBlockDesc);
      ...
      //copy blocks
      std::copy(blocks_-meta_->blocks_cnt_+1,blocks_+1,(NZBlockDesc*)(locTmpPtr+offset_block));
