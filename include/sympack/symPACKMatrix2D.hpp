...
namespace symPACK{
  ...
  template <typename colptr_t, typename rowind_t, typename T, typename int_t = int> 
    class blockCell_t: public blockCellBase_t {
	  ...
	  _gstorage(nullptr),_nnz(0), _nnz_comm(0), _storage_size(0), _own_storage(true) {}
        ...
        blockCell_t (  int_t i, int_t j,upcxx::global_ptr<char, upcxx::memory_kind::cuda_device> d_ext_storage, 
          ...
          //cudaMemcpy(symPACK::gpu_allocator.local(_d_nzval), _nzval, sizeof(T)*_nnz, cudaMemcpyHostToDevice);
	...
	void dump_nnz(std::string prelude) {
	  ...
	  logfileptr->OFS()<<"Checking factorize pre"<<std::endl;
          upcxx::copy(diag_nzval, actual_nzval, snode_size*snode_size).wait();
          check_close(correct_nzval, actual_nzval, snode_size*snode_size);
	  ...
	  logfileptr->OFS()<<"Checking factorize post"<<std::endl;
          upcxx::copy(diag_nzval, actual_nzval, snode_size*snode_size).wait();
          check_close(correct_nzval, actual_nzval, snode_size*snode_size);
          delete actual_nzval;
...
//#endif
	      ...
	      tmpBuffers.dev_resize(src_nrows, tmpBuffers.d_src_to_tgt_offset, tmpBuffers.d_offset_size);
              tmpBuffers.src_to_tgt_offset.resize(src_nrows);//d
#else
...
#pragma unroll
		...
		//logfileptr->OFS()<<"Checking tgt update"<<std::endl;
		upcxx::copy(tgt2, tgt, this->_nnz).wait();
...
#ifdef CUDA_MODE
	      logfileptr->OFS()<<"Checking pre axpy"<<std::endl;
	      check_close(tgt2+row*ldsol, tgt+row*ldsol, ldsol);
	      check_close(src2+row*ldsol, src+row*ldsol, ldsol);
	  ...
	  logfileptr->OFS()<<"Checking back_update axpy"<<std::endl;
	  check_close(this->_nzval, this->_d_nzval, src_cell.nnz());
#else          
	  ...
	  CUDA_ERROR_CHECK(cudaDeviceSynchronize());
	    ...
	    auto contrib_nzval = tgt_contrib._nzval;
	      ...
	      T * src2 = diag_contrib._nzval + block.offset + (fact_block.first_row - block.first_row)*ldsol;
              T * fact2 = this->_nzval+fact_block.offset;
#else
	      ...
	      blas::Gemm('N','T',ldsol,ldfact,this->block_nrows(fact_block), 
              T(-1.0),src2,ldsol,fact2,ldfact,T(1.0),tgt_contrib._nzval,ldsol);
	      logfileptr->OFS()<<"Checking BUC GEMM Result"<<std::endl;
	      check_close(tgt_contrib._nzval, tgt_contrib._d_nzval, tgt_contrib._nnz);
#else              
              blas::Gemm('N','T',ldsol,ldfact,this->block_nrows(fact_block), 
                                ...
                                if ( data ) {
                                  data->on_fetch_future = data->on_fetch_future.then(
                                      ...
                                      if (matptr->options_.decomposition == DecompositionType::LDL) {
                                        ...
                                        logfileptr->OFS()<<"FUC Case 1"<<std::endl;
#ifdef CUDA_MODE
				  ...
				  data->d_remote_gptr = d_gptr;
                                  ...
                                  if ( data ) {
                                    ...
                                    data->on_fetch_future = data->on_fetch_future.then(
                                        ...
                                        logfileptr->OFS()<<"FUC Case 2"<<std::endl;
                                        pdata->extra_data = std::shared_ptr<blockCellBase_t>( (blockCellBase_t*)new snodeBlock_t(I,I,pdata->d_landing_zone, pdata->landing_zone,fc,width,nnz,nblocks) );
#else                                        
					pdata->extra_data = std::shared_ptr<blockCellBase_t>( (blockCellBase_t*)new snodeBlock_t(I,I,pdata->landing_zone,fc,width,nnz,nblocks) );
#endif
...
#ifdef CUDA_MODE
				data->d_remote_gptr = d_gptr;
				logfileptr->OFS()<<"null data"<<std::endl;
#endif
                                }
...
#ifdef CUDA_MODE
				      logfileptr->OFS()<<"BUC Case 1"<<std::endl;
                                  ...
                                  if ( data ) {
                                    ...
                                    data->on_fetch_future = data->on_fetch_future.then(
                                        ...
                                        if (matptr->options_.decomposition == DecompositionType::LDL) {
                                          ...
                                          logfileptr->OFS()<<"BUC Case 2"<<std::endl;
#ifdef CUDA_MODE
                                          pdata->extra_data = std::shared_ptr<blockCellBase_t>( (blockCellBase_t*)new snodeBlock_t(I,I,pdata->d_landing_zone, pdata->landing_zone,fc,width,nnz,nblocks) );
#else
                                          pdata->extra_data = std::shared_ptr<blockCellBase_t>( (blockCellBase_t*)new snodeBlock_t(I,I,pdata->landing_zone,fc,width,nnz,nblocks) );
