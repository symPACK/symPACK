#ifndef _SYMPACK_SP_BLAS_HEADER_
#define _SYMPACK_SP_BLAS_HEADER_
...
sp_dgemv_dist(char *trans, MYSCALAR alpha, DistSparseMatrix<MYSCALAR> & A, MYSCALAR *x, 
    int incx, MYSCALAR beta, MYSCALAR *y, int incy, MYSCALAR * ytmp)
{
...
//        for(i=0; i<np;++i){
//          if(i!=iam){
//            blas::Axpy(leny,SYMPACK::ONE<MYSCALAR>(),&recv_buffer[i*leny],1,y,1);
//          }
//        }
...
//          blas::Axpy(leny,SYMPACK::ONE<MYSCALAR>(),yrecv,1,ytmp,1);
//        }
////        std::copy(ytmp,ytmp+leny,y);
////        blas::Axpy(leny,SYMPACK::ONE<MYSCALAR>(),ytmp,1,y,1);
//        delete [] yrecv;
//      }
//      else{
    ...
    MYSCALAR beta, MYSCALAR *c, int ldc)
{
  ...
  bool notran = true;//lsame_(trans, "N");
  if (notran) {
    lenx = A.size;
    leny = lenx;
  } else {
  ...
  MYSCALAR * ytmp = new MYSCALAR[leny];

  for (j = 0; j < n; ++j) {
    sp_dgemv_dist(transa, alpha, A, &b[ldb*j], incx, beta, &c[ldc*j], incy, ytmp);
...
#endif // _SYMPACK_SP_BLAS_HEADER_
